                                             ...
                                             , const int trigger):WDcfg(),WDrun()// do I give the system trigger?
{
    ...
    std::ofstream handlefileout("handle");
...
int daq_device_caen_v1730::put_data(const int etype, int * adr, const int length )
{
    ...
    if (ElapsedTime > 1000) {
        ...
        if (Nb == 0)
            //printf("No data...\n");
            cout<<"No data...\r"<<flush;
        else
            //printf("Reading at %.2f MB/s (Trg Rate: %.2f Hz)\n", ReadoutRate, TriggerRate);
            cout<<"Reading at "<<ReadoutRate<<" MB/s (Trg Rate: "<<TriggerRate<<" Hz)\r"<<flush;
    ...
    for(int i = 0; i < (int)NumEvents; i++) {
...
#ifdef MAX_NEVT_FIFO_SEND
        if (EventInfo.EventCounter-fifocal_prevEventCounter>MAX_NEVT_FIFO_SEND){
            addfifo(EventInfo.EventCounter);
            fifocal_prevEventCounter=EventInfo.EventCounter;
        }
#endif
        /* Fill V1730 data (Event16) with rcdaq data stream */
        *d++=EventInfo.BoardId;
        ipos++;
        ...
        *d++=EventInfo.TriggerTimeTag;
        //cout<<EventInfo.TriggerTimeTag<<endl;
        ipos++;
        memcpy(d++,Event16->ChSize,BoardInfo.Channels*sizeof(uint32_t));
        ipos+=BoardInfo.Channels;
        ipos_prev = BoardInfo.Channels -1; // -1 bit is nescessary!
        ...
        for(int ch=0; ch<(int32_t)BoardInfo.Channels; ch++) {
            ...
            if (WDcfg.Nbit == 8){
                ...
                if (BoardInfo.FamilyCode != CAEN_DGTZ_XX742_FAMILY_CODE) {
                    memcpy(d+=ipos_prev,Event16->DataChannel[ch],Event16->ChSize[ch]*sizeof(uint16_t));
                    ipos_prev = Event16->ChSize[ch]/2+Event16->ChSize[ch]%2;
                    ipos+=ipos_prev;
                }
                else {
        ...
        //! not sure if CAEN_DGTZ_FreeEvent is needed:
//        ret = CAEN_DGTZ_FreeEvent(handle,(void**)&Event16);
//        if (ret) {
//            cout<<"CAEN_DGTZ_FreeEvent Error"<<endl;
//            exit(0);
//        }
    }
